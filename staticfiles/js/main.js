...
function initializeImageSearch() {
    ...
    searchByImageBtn.addEventListener('click', async () => {
        const file = searchImageInput.files?.[0];
        if (!file) {
            setStatus('Please select an image before searching.', 'warning');
        ...
        setStatus('Analyzing image and finding visual matches...');
        ...
        try {
            ...
            if (data.success) {
                setStatus(data.message || 'Image search complete.', 'success');
                showNotification(data.message || 'Image search complete.', 'success');
                if (data.redirect_url) {
                    window.location.href = data.redirect_url;
                }
            } else {
                setStatus(data.message || 'Unable to complete image search.', 'danger');
...
function initializeAiConcierge() {
    ...
    const captureLeadData = (message) => {
        const budgetMatch = message.match(/(?:rs\.?|inr|â‚¹)?\s?(\d+(?:[.,]\d+)?)\s*(k|thousand|lakh|lac|m|million|cr|crore)?/i);
